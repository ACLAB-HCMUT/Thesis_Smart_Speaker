...
# partner_ip = "192.168.137.128"  # IP Address c?a máy kia
port_send = 8080
port_receive = 9090 
...
def send_audio(partner_ip):
    ...
    print(f"?? G?i âm thanh d?n {partner_ip}:{port_send}...")
    ...
    while True:
        try:
            ...
            print(f"? L?i g?i âm thanh: {e}")
            break

def receive_audio():
    udp_socket = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    udp_socket.bind(("0.0.0.0", port_receive))
    print(f"?? Nh?n âm thanh trên c?ng {port_receive}...")
